...
export default function ZonesOverviewTab({ data }: { data: RoutesData }) {
  const { fbos, zones, routeDefinitions, isPending, bulkRedetectZones, overrideFboZone } = data;
  ...
  const routesByZone = useMemo(() => {
    ...
    for (const rd of routeDefinitions) { if (rd.zone_id) m.set(rd.zone_id, (m.get(rd.zone_id) ?? 0) + 1); }
    return m;
  }, [routeDefinitions]);
  ...
  async function handleOverrideSubmit() {
    ...
    await overrideFboZone(overrideTarget, overrideZoneId);
    setOverrideTarget(null); setOverrideZoneId("");
  }
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-xl shadow-gray-200/50 overflow-hidden">
        <div className="flex items-center justify-between p-5 border-b border-gray-100 bg-gray-50/60">
          <div>
            ...
            <p className="text-[11px] text-gray-400 font-medium mt-0.5">5 Main Zones (North, Central, West, South, East)</p>
          </div>
                ...
                <div key={zoneName} className="rounded-2xl border border-gray-100 overflow-hidden hover:shadow-md transition-all">
                  ...
                  <div className="p-4">
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <div>
                        <h3 className="font-black text-gray-900 text-sm">{zoneName} Zone</h3>
                        <p className="text-[10px] font-mono text-gray-400">Corporation {config.corporationNumber}</p>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-2 mb-3">
                      {[{ l: "FBOs", v: zoneFbos.length }, { l: "Routes", v: routeCount }].map(({ l, v }) => (
                    ...
                    <div className="p-2.5 rounded-xl bg-gray-50 border border-gray-100">
                      <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">Key Areas</p>
                      <p className="text-[11px] text-gray-600 font-medium leading-relaxed">{config.areas.join(", ")}</p>
                    </div>
                  </div>
                </div>
              );
            ...
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredFbos.slice(0, 50).map((fbo) => {
                  ...
                  return (
                    <tr key={fbo.id} className={`hover:bg-gray-50 transition-colors ${isOverriding ? "bg-blue-50/40" : ""}`}>
                      ...
                      <td className="px-4 py-3 text-gray-500 max-w-[220px]"><span className="truncate block">{fbo.address ?? "—"}</span></td>
                      <td className="px-4 py-3">
                        {isOverriding ? (
                          <select className="text-xs font-semibold bg-white border border-blue-300 rounded-lg px-2 py-1.5 text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500/20 w-full cursor-pointer"
                            value={overrideZoneId} onChange={(e) => setOverrideZoneId(e.target.value)}>
                      ...
                      <td className="px-4 py-3">
                        {isOverriding ? (
                          ...
                          <div className="flex items-center gap-1">
                            <button onClick={() => { setOverrideTarget(fbo.id); setOverrideZoneId(fbo.zone_id ?? ""); }}
                ...
                {filteredFbos.length === 0 && (
                  <tr><td colSpan={5} className="px-4 py-10 text-center text-gray-400 text-xs font-medium">No FBOs match your filter.</td></tr>
                )}
              </tbody>
            </table>
