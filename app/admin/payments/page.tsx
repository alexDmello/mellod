  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="card p-5 bg-white border border-amber-200 shadow-sm flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-amber-100 text-amber-700 flex items-center justify-center flex-shrink-0 font-bold">
            <IndianRupee className="w-6 h-6" />
          </div>
          <div>
            <p className="text-xs text-gray-500 font-semibold uppercase tracking-wider">Total Pending Payouts</p>
            <p className="text-2xl font-extrabold text-amber-900 mt-0.5">{formatCurrency(totalUnpaidBalance)}</p>
            <p className="text-[11px] text-amber-700 mt-0.5">{pendingSummaries.length} FBO(s) awaiting disbursement</p>
          </div>
        </div>

        <div className="card p-5 bg-white border border-blue-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-gray-500 font-semibold uppercase tracking-wider">Unsettled Volume</p>
            <p className="text-2xl font-extrabold text-gray-900 mt-0.5">{formatLiters(totalUnpaidLiters)}</p>
            <p className="text-[11px] text-gray-500 mt-0.5">Verified UCO collected</p>
          </div>
        </div>

        <div className="card p-5 bg-white border border-green-100 shadow-sm flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-green-100 text-green-700 flex items-center justify-center flex-shrink-0 font-bold">
            <CheckCircle2 className="w-6 h-6" />
          </div>
          <div>
            <p className="text-xs text-gray-500 font-semibold uppercase tracking-wider">Total Settled Disbursements</p>
            <p className="text-2xl font-extrabold text-green-800 mt-0.5">{formatCurrency(totalDisbursedAmount)}</p>
            <p className="text-[11px] text-green-700 mt-0.5">{receipts.length} Payment Receipts issued</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-200 pb-3">
        ...
        <div className="relative min-w-[240px]">
          ...
          <input
            ...
            className="form-input !pl-8 !py-1.5 text-xs bg-white border-gray-200"
            value={searchQuery}
      ...
      {/* Loading state */}
      {loading ? (
        <div className="p-16 text-center text-gray-400 flex flex-col items-center gap-2 bg-white rounded-2xl border border-gray-100">
          <Loader2 className="w-8 h-8 animate-spin text-green-700" />
          <span>Calculating FBO payment balances...</span>
        </div>
      ) : (
        <>
            ...
            <div className="space-y-4">
              {filteredPending.length === 0 ? (
                <div className="card p-12 text-center text-gray-400 bg-white">
                  ...
                  <p className="font-semibold text-gray-700">All verified pickups are fully settled!</p>
                  <p className="text-xs text-gray-400 mt-1">There are no pending balances awaiting payout to FBOs.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 gap-4">
                  {filteredPending.map((item) => (
                    <div key={item.fbo.id} className="card p-5 bg-white border border-gray-200 shadow-sm hover:border-green-300 transition-all space-y-4">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-100 pb-3">
                        ...
                        <div className="flex items-center gap-4 bg-amber-50/80 px-4 py-2.5 rounded-xl border border-amber-100">
                          <div>
                            <span className="text-[10px] text-amber-800 font-bold uppercase tracking-wider block">Total Amount Due</span>
                            <span className="text-lg font-black text-amber-900">{formatCurrency(item.unpaidAmount)}</span>
                          </div>
                          <div className="h-8 w-px bg-amber-200" />
                          <div>
                            <span className="text-[10px] text-amber-800 font-bold uppercase tracking-wider block">Volume</span>
                            <span className="text-sm font-extrabold text-amber-900">{formatLiters(item.unpaidLiters)}</span>
                          </div>
                          <div className="h-8 w-px bg-amber-200" />
                          <div>
                            <span className="text-[10px] text-amber-800 font-bold uppercase tracking-wider block">Pickups</span>
                            <span className="text-sm font-extrabold text-amber-900">{item.unpaidPickups.length} entries</span>
                          </div>
                        </div>
                      </div>

                      {/* Payment Bank / UPI Info Badge */}
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs bg-gray-50 p-3 rounded-xl">
                        <div>
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="btn btn-secondary text-xs py-1.5 px-3 bg-white hover:bg-gray-100 border border-gray-200 text-gray-800 font-semibold flex items-center gap-1"
                          >
                            <Eye className="w-3.5 h-3.5 text-blue-600" />
                            View Pickup Entries ({item.unpaidPickups.length})
                          </button>
                          ...
                          <button
                            ...
                            className="btn btn-primary text-xs py-1.5 px-3.5 font-bold flex items-center gap-1 shadow-sm"
                          >
                            <CreditCard className="w-3.5 h-3.5" />
                            Disburse Payment & Issue Statement
                          </button>
                        </div>
                      </div>
                    </div>
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-gray-50 text-gray-600 text-xs border-b border-gray-200 font-bold uppercase tracking-wider">
                        ...
                        <th className="p-3.5">FBO Supplier</th>
                    ...
                    <tbody className="divide-y divide-gray-100 text-xs">
                      {filteredReceipts.map((r) => (
                        <tr key={r.id} className="hover:bg-gray-50/80 transition-colors">
                          <td className="p-3.5 font-mono font-bold text-green-800">
                            {r.receipt_number}
                          </td>
                          <td className="p-3.5">
                          ...
                          <td className="p-3.5 text-right font-extrabold text-green-800 text-sm">
                            {formatCurrency(Number(r.amount))}
                          </td>
                          <td className="p-3.5">
                            <span className="capitalize font-semibold text-gray-700 bg-gray-100 px-2 py-0.5 rounded text-[11px]">
                              {r.payment_method}
                            </span>
                            {r.reference_number && (
                          ...
                          <td className="p-3.5 text-center">
                            <button
                              ...
                              className="btn btn-secondary text-xs py-1 px-2.5 bg-white border border-gray-200 text-gray-700 hover:text-gray-900 flex items-center justify-center gap-1 mx-auto font-semibold"
                            >
      ...
      {/* MODAL 1: VIEW ENTRIES INCLUDED IN UNPAID BALANCE */}
      {viewEntriesFbo && (
        <div className="fixed inset-0 z-50 bg-black/75 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-3xl w-full max-h-[85vh] flex flex-col overflow-hidden shadow-2xl space-y-0 border border-gray-200">
                  ...
                  <div key={p.id} className="p-4 bg-gray-50 rounded-xl border border-gray-200 flex items-center justify-between text-xs">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="badge bg-green-100 text-green-800 text-[10px] uppercase font-bold">
                          Verified ✓
                        </span>
                      </div>
                    ...
                    <div className="text-right">
                      ...
                      <span className="font-black text-green-700 text-sm">{formatCurrency(Number(p.total_amount))}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full overflow-hidden shadow-2xl space-y-0 border border-gray-200">
            ...
            <form onSubmit={handleConfirmPayout} className="p-5 space-y-4 text-xs">
              <div className="p-3.5 bg-green-50 rounded-xl border border-green-200 space-y-1">
                <p className="font-extrabold text-green-900 text-sm">{payoutFbo.fbo.business_name}</p>
                <div className="flex items-center justify-between pt-1 border-t border-green-200 text-green-800 font-semibold">
                  <span>Disbursing Amount:</span>
                  <span className="text-base font-black font-mono">{formatCurrency(payoutFbo.unpaidAmount)}</span>
                </div>
                <div className="flex items-center justify-between text-green-700 text-[11px]">
              ...
              <div className="pt-3 border-t border-gray-100 flex justify-end gap-2">
                ...
                <button
                  ...
                  className="btn btn-primary text-xs px-5 py-2 font-bold flex items-center gap-1.5 shadow-sm"
                >
                  {submittingPay ? (
                    <>
