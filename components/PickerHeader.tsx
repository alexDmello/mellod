...
export default function PickerHeader({ children, subtitle = "Field Agent App", showBack = false }: PickerHeaderProps) {
  ...
  return (
    ...
    >
      ...
      {/* Top Branding & Logout/Back Bar */}
      <div className="flex items-center justify-between pb-4 border-b border-white/15 mb-4 relative z-10">
        <div className="flex items-center gap-3">
          {showBack && (
            <button
              onClick={() => router.back()}
              className="w-9 h-9 rounded-xl bg-white/15 hover:bg-white/25 backdrop-blur-md text-white flex items-center justify-center transition-all border border-white/20 shadow-sm mr-1 cursor-pointer"
              aria-label="Go back"
            >
